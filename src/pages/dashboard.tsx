...
import { getContributions } from 'apiSdk/contributions';
...
function DashboardPage() {
  ...
  const contributionStat = useSWR<PaginatedInterface<ContributionInterface>>(
    () => `/contributions`,
    () => getContributions({ limit: 1, offset: 0 }),
  );
  ...
  const instructorStat = useSWR<PaginatedInterface<InstructorInterface>>(
    () => `/instructors`,
    () => getInstructors({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Contributions',
      result: contributionStat?.data?.totalCount,
      isLoading: contributionStat.isLoading,
    ...
    {
      title: 'Instructors',
      result: instructorStat?.data?.totalCount,
      isLoading: instructorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
