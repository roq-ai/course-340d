...
type ColumnType = ColumnDef<InstructorInterface, unknown>;
...
export function InstructorListPage(props: InstructorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InstructorInterface>>(
    () => `/instructors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InstructorInterface) => {
    if (hasAccess('instructor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/instructors/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/instructors/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
